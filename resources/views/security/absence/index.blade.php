...
@section('content')
                        ...
                        <div class="card">
                            <div class="card-body loader-area">
                                ...
                                <div class="table-responsive">
                                    <table id="absenceTable" class="table table-hover">
                                        <thead>
                                            <tr>
                                                <th style="padding: 10px 10px;">No.</th>
                                                <th style="padding: 10px 10px;">Tanggal</th>
                                                <th style="padding: 10px 10px;">Shift   </th>
                                                <th style="padding: 10px 10px;">Catatan</th>
                                                <th style="padding: 10px 10px;">Laporan Kejadian</th>
    ...
    <!-- IMPORTANT LINK -->
    <a href="{{ route('absence.getDatatables') }}" id="absenceGetDatatables"></a>
    <!-- /IMPORTANT LINK -->
@endsection
    ...
    <script>
        $(document).ready(function() {
            ...
            function updateExportLink(start, end) {
                $('#exportpdf').data('href', '/security/report/' + start.format('YYYY-MM-DD') + '+' + end.format('YYYY-MM-DD'));
            }
            ...
            $.extend($.fn.dataTable.defaults, {
                ...
                initComplete: function() {
                    var $searchInput = $('#absenceTable_filter input').addClass('form-control form-control-sm').attr('placeholder', 'Cari Tanggal...');
                    $searchInput.parent().addClass('d-flex align-items-center');

                    var $lengthMenu = $('#absenceTable_length select').addClass('form-control form-control-sm');

                    $lengthMenu.parent().addClass('d-flex align-items-center');
                    
                    $('#absenceTable_length').addClass('d-flex align-items-center');
                }
            });

            var url = $('#absenceGetDatatables').attr('href');
            var table = $('#absenceTable').DataTable({
                ...
                columns: [
                    ...
                    {data: 'shift', name: 'shift', className: 'align-middle'},
                    {data: 'details', name: 'details', className: 'align-middle'},
                    {data: 'incidentReports', name: 'incidentReports', className: 'align-middle'},
            ...
            $('#created_at').on('apply.daterangepicker', function(ev, picker) {
                ...
                table.ajax.url('/security/getDatatables?date=' + startDate + ' - ' + endDate).load();
